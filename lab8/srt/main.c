...
COLORS_thread gimpCity[width * height];
int nthreads;
scene_t scene;

void* threadFunction(void *threadnum)
{
	int thread = *((int*)threadnum);
	int cols = width / nthreads;
	int currentIndex = (thread)*cols;

	int maxPerThread = currentIndex + cols;

	if (thread + 1 == nthreads)
		maxPerThread += (width % cols);
	...
	/* for every pixel */
	for (int px = currentIndex; px<maxPerThread; ++px)
	{
		...
		for (int py = 0; py<height; ++py)
		{
			...
			*/
			...
			COLORS_thread cur;
			cur.red = scaled_color[0];
			cur.green = scaled_color[1];
			cur.blue = scaled_color[2];

			globalArrayIndex = px * height + py;
			gimpCity[globalArrayIndex] = cur;
		}
	}
	return NULL;
}
